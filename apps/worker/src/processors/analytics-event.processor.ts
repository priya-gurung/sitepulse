...
export async function processAnalyticsEvent(event: QueuedEvent): Promise<void> {
  await tracer.startActiveSpan("worker.process_event", async (span) => {
    ...
    try {
      ...
      const country =
        typeof event.props?._country === "string" ? (event.props._country as string) : null;
      ...
      const { session, isNewSession } = await prisma.$transaction(async (tx) => {
        const visitor = await tx.visitor.upsert({
          where: {
            siteId_visitorHash: { siteId: event.siteId, visitorHash: event.visitorHash },
          },
        ...
        const activeSession = await tx.session.findFirst({
          where: {
            ...
            siteId: event.siteId,
            endedAt: null,
            lastActiveAt: { gte: new Date(now.getTime() - SESSION_IDLE_TIMEOUT_MS) },
          },
      ...
      eventProcessingDuration.record(performance.now() - start, { event_type: event.type });
      span.end();
    }
  });
}
