...
export async function processAnalyticsEvent(event: QueuedEvent): Promise<void> {
  await tracer.startActiveSpan("worker.process_event", async (span) => {
    ...
    try {
      ...
      logger.info("Starting analytics event processing", {
        siteId: event.siteId,
      ...
      const { session, isNewSession } = await tracer.startActiveSpan(
        "worker.postgres_transaction",
        async (txSpan) => {
          txSpan.setAttribute("site.id", event.siteId);
          try {
      ...
      await tracer.startActiveSpan("worker.tinybird_ingest", async (tbSpan) => {
        tbSpan.setAttribute("site.id", event.siteId);
        tbSpan.setAttribute("event.id", eventId);
        tbSpan.setAttribute("session.id", session.id);
        try {
          await ingestEvent({
            event_id: eventId,
      ...
      logger.info("Successfully processed and ingested analytics event", {
        siteId: event.siteId,
        eventId,
